...
void main(){
  ...
  for (int i = 0; i < CIRCLES; i++){
    float m = min(smoothstep(dist+1.,dist,manhattanDistance(uv,vec2(circlesx[i],circlesy[i]))),smoothstep(dist-lwidth,dist-lwidth+1.,manhattanDistance(uv,vec2(circlesx[i],circlesy[i]))));
